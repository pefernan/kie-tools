...
export class SwfServiceCatalogStore {
  ...
  public get needsAuth() {
    return Array.from(this.catalogSourcesStore.keys()).find((source) => source.needsAuth()) != undefined;
  }
  ...
  private initSources(sources: SwfServiceCatalogSource[]): Promise<void> {
    ...
    sources.forEach((source) => {
      ...
      source.subscribeToChange((services) => this.refreshSourceStore(source, services));
    });
  ...
  private async refreshSourceStore(catalogSource: SwfServiceCatalogSource, services: SwfServiceCatalogService[]) {
